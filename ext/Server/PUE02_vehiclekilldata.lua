...
--
	print ("") 
	if global_kills >= 1 then print ("Found player "..data_playername.." in KILL table, with the "..s_weapon.." - Kills:"..r_killdata)
		global_kills = 0
		end

	do updateweaponkillstats(player,data_playername)
	end
...
-- Here we do some function calls
	...
	WeaponResults = SQL:Query('SELECT Weaponname, Soldiername FROM '..s_table..' WHERE Soldiername = ? and Weaponname = ?', data_playername, s_weapon)
      if not WeaponResults then print('Failed to read "..s_table.." KILL query: ' .. SQL:Error()) ; return end
...
-- New player stuff
	if founddata_Soldiername ~= data_playername and founddata_Weaponname ~= s_weapon then print (s3.." - Inserting "..data_playername.." with "..r_killdata.." kills in "..s_table)
