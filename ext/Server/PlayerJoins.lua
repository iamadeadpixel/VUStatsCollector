...
-- Players ID get checked by its Guid
	...
	print("*** SQL check: All rows readed, lets check for the correct player data. ***"); 
		...
		if s_accountGuid == temp_Guid then print("*** ServerInfo: GUID FOUND FOR PLAYER: " ..s_player.. " - :" .. temp_Guid)
--		if s_playerGuid  == temp_Guid then print("*** ServerInfo: GUID FOUND FOR PLAYER: " ..s_player.. " - :" .. temp_Guid)
	...
	if temp_PlayerName == s_player then print("*** UpdateCheckInfo: Players name not changed: " ..temp_PlayerName.. " - "..s_player )
...
--
	...
	elseif temp_PlayerName ~= s_player then print("*** UpdateCheckInfo: Players name was changed from " ..temp_PlayerName.. " into "..s_player )
...
-- Here we check the last ServerPlayers entry
	...
	if not SQL:Query('UPDATE tbl_serverstats SET SumPlayers = SumPlayers + ? ', 1) then print('Failed to execute Update query: ' .. SQL:Error()); return end

	print("*** Reading all ServerPlayers ***"); 
	local ServerPlayers_results = SQL:Query ('SELECT SumPlayers FROM tbl_serverstats')
		if not ServerPlayers_results then print('Failed to read Guid query: ' .. SQL:Error()) ; return end
