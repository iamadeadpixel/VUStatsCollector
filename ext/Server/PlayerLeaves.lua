...
Events:Subscribe('Player:Left', function(player)
	...
	print("*** UpdateCheckInfo: Human player " ..getnamehuman[player.name] .. " leaving the server, removing from SQL")


	if CountPlayers == 0 then print("*** UpdateCheckInfo: No more human players on the server"); end
	...
	if Session_PlayTime_End[player.name] then
		print("*** UpdateCheckInfo: Player playtime on logout in seconds : " ..
	...
	print("*** UpdateCheckInfo: SQL check - All rows readed, lets check for the correct player data. ***");
	...
	for _, l_Row in pairs(guid_results) do
		...
		if s_accountGuid == temp_Guid then print("*** UpdateCheckInfo: GUID FOUND FOR LEAVING PLAYER: " .. getnamehuman[player.name] .. " - :" .. temp_Guid) -- Using the Accounts Guid
			...
			print("*** UpdateCheckInfo: Human player "..getnamehuman[player.name].." leaving the server, removing from DB")
		...
		Mcom_Destroyed[player.name] = {}
			...
			print("*** UpdateCheckInfo: Player has donked out of the server ***");
		end
	end
end)
