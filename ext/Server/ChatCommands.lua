...
Events:Subscribe('Player:Chat', function(player, recipientMask, message)
	...
	if s_topSelect == "all" then
		CHATresults = SQL:Query('SELECT Weaponname, Soldiername, Kills FROM ' ..
			s_table .. ' ORDER BY Kills DESC, Weaponname ASC, Soldiername ASC LIMIT 3 ')
		if not CHATresults then
			print('Failed to got data query: ' .. SQL:Error()); return
		end
		print("ALL selected")
	elseif s_topSelect == "player" then
		CHATresults = SQL:Query(
			'SELECT Weaponname, Soldiername, Kills FROM ' .. s_table ..
			'  WHERE Soldiername = ? ORDER BY Kills DESC LIMIT 3 ', player.name)
		if not CHATresults then
			print('Failed to got data query: ' .. SQL:Error()); return
		end
...
Events:Subscribe('Player:Chat', function(player, recipientMask, message)
	...
	if s_mapSelect == "all" then
		CHATresults = SQL:Query('SELECT MapName, Gamemode, Roundscore, Roundtime, winningTeam FROM ' ..
			s_table .. ' ORDER BY Roundscore DESC LIMIT 3 ')
		if not CHATresults then
			print('Failed to got data query: ' .. SQL:Error()); return
		end
		print("ALL selected")
	elseif s_mapSelect == "map" then
		CHATresults = SQL:Query(
			'SELECT MapName, Gamemode, Roundscore, Roundtime, winningTeam FROM ' ..
			s_table .. ' WHERE Gamemode = ? ORDER BY Roundscore DESC LIMIT 3 ', s_gametype)
		if not CHATresults then
			print('Failed to got data query: ' .. SQL:Error()); return
		end
		print("MAP selected")
