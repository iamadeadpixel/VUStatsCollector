...
-- --------------------------------------
	...
	local s_Results = SQL:Query('SELECT * FROM tbl_headshots')
	if not s_Results then
		print("*** Creating Table: 'tbl_headshots' ***")
		s_Query = [[
	CREATE TABLE IF NOT EXISTS `tbl_headshots` (
	`Soldiername` TEXT,
	`Headshots_Killer` INTEGER,
	`Headshots_Victim` INTEGER
	);
		]]

		if not SQL:Query(s_Query) then
			print('Failed to execute query for tbl_headshots Table: ' .. SQL:Error())
			return
		end
	end
	...
	local s_Results = SQL:Query('SELECT * FROM tbl_teamkilled')
	if not s_Results then
		print("*** Creating Table: 'tbl_teamkilled' ***")
		s_Query = [[
	CREATE TABLE IF NOT EXISTS `tbl_teamkilled` (
	`Soldiername` TEXT,
	`TeamKilled_Killer` INTEGER,
	`TeamKilled_Victim` INTEGER
	);
		]]

		if not SQL:Query(s_Query) then
			print('Failed to execute query for tbl_teamkilled Table: ' .. SQL:Error())
			return
		end
	end
	...
	local s_Results = SQL:Query('SELECT * FROM tbl_dogtags')
	if not s_Results then
		print("*** Creating Table: 'tbl_dogtags' ***")
		s_Query = [[
	CREATE TABLE IF NOT EXISTS `tbl_dogtags` (
	`Soldiername` TEXT,
	`Dogtags_Killer` INTEGER,
	`Dogtags_Victim` INTEGER
	);
		]]

		if not SQL:Query(s_Query) then
			print('Failed to execute query for tbl_dogtags Table: ' .. SQL:Error())
			return
		end
	end
	...
	local s_Results = SQL:Query('SELECT * FROM tbl_revives')
	if not s_Results then
		print("*** Creating Table: 'tbl_revives' ***")
		s_Query = [[
	CREATE TABLE IF NOT EXISTS `tbl_revives` (
	`Soldiername` TEXT,
	`Revives_Give` INTEGER,
	`Revives_Take` INTEGER
	);
		]]

		if not SQL:Query(s_Query) then
			print('Failed to execute query for tbl_revives Table: ' .. SQL:Error())
			return
		end
	end
