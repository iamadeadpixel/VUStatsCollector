...
function updateweaponstats(player, data_playername)
	...
	print ("POST Update record for "..data_playername.." - Table name:"..s_table.." - Weapon:"..s_weapon.." - Kills:"..r_killdata.." - Headshots:"..r_headshotdata.." - Deaths:"..r_deathdata)
	end
...
function updateweaponstatssolo(player, data_playername)

	if getnameall[data_playername] and r_var1data >= 1 then
	if Config.consolespam_functioncalls_msg then
			print("POST Found player "..data_playername.." in "..s_table.." table, for table entry "..var1data.." - Data:"..r_var1data)
	end

			if not SQL:Query('UPDATE '..s_table..' SET '..var1data..' = '..var1data..' + ? WHERE Soldiername = ?', r_var1data, data_playername) then
